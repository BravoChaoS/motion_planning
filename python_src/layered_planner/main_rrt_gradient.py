...
class Params:
    def __init__(self):
        self.mode = 1  # 0 - RRT, 1 - RRT + APF, 2 - RRT + APF + SA
        ...
        self.maxiters = 2000  # max number of samples to build the RRT
...
if __name__ == '__main__':
    ...
    for i in range(params.test_loops):
        ...
        suc = False
        if params.mode == 0:
            P_long, t, it, suc = rrt_path(obstacles, xy_start, xy_goal, params)
        elif params.mode == 1:
            f = combined_potential(obstacles_grid, xy_goal, params.influence_radius)
            [gy, gx] = np.gradient(-f)
            draw_gradient(f)
            # plt.show()
            P_long, t, it, suc = rrt_apf_path(obstacles, xy_start, xy_goal, params, gx, gy)
        elif params.mode == 2:
            ...
            draw_gradient(att)
            draw_gradient(rep)
            P_long, t, it, suc = rrt_apf_sa_path(obstacles, xy_start, xy_goal, params, g_att, g_rep)
        if suc:
            tms.append(t)
            iters.append(it)
            succeed = succeed + 1
    ...
    # 成功率
    print('Success rate: %', succeed * 100. / params.test_loops)
    ...
    with open(file_path, mode='w', encoding='utf-8') as f:
        ...
        # 成功率
        f.write('Success rate: %' + str(succeed * 100. / params.test_loops) + '\n')
